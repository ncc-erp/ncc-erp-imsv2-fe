...
import { getEventInformations } from "@/store/news/thunkApi"
...
import { updateLikeEventInformations } from "@/store/news"
import { WIDGET_PADDING, WIDGET_SPACING, WIDGET_WIDTH } from "../GridLayout"
import Link from '@mui/material/Link';

interface IWidgetEventInformation {
  pEventInformations: INewsRes[]
  pGetEventInformations: () => Promise<unknown>
  row?: number
}
...
const WidgetEventInformations = ({ pGetEventInformations, pEventInformations, row = 0 }: IWidgetEventInformation) => {
  const classes = useStyles()
  const dispatch = useDispatch()
  const swiperRef = useRef<SwiperCore | null>(null)

  useEffect(() => {
    pGetEventInformations()
  }, [])
  const handleUpdateLike = useCallback(
    async (
      ...
      e.stopPropagation()
      const index = pEventInformations.findIndex((el) => el.id === news.id)
      if (index !== -1) {
        dispatch(updateLikeEventInformations(index))
        const res = await updateLike({
          id: news.id,
          liked: !pEventInformations[index].hasCurrentUserLiked
        })
      }
    },
    [pEventInformations]
...
const mapStateToProps = (state: AppState) => ({
  pEventInformations: state.news.eventInformations
})

const mapDispatchToProps = (dispatch: AppDispatch) => ({
  pGetEventInformations: () => dispatch(getEventInformations())
})

export default connect(mapStateToProps, mapDispatchToProps)(WidgetEventInformations)
