...
import { getNewsInformations } from "@/store/news/thunkApi"
...
import { updateLikeNewsInformations } from "@/store/news"
import { WIDGET_PADDING, WIDGET_SPACING, WIDGET_WIDTH } from "../GridLayout"
import Link from '@mui/material/Link';

interface IWidgetNewsInformations {
  pNewsInformations: INewsRes[]
  pGetNewsInformations: () => Promise<unknown>
  row?: number
}
...
const WidgetNewsInformations = ({ pGetNewsInformations, pNewsInformations, row = 0 }: IWidgetNewsInformations) => {
  const classes = useStyles()
  const dispatch = useDispatch()
  const swiperRef = useRef<SwiperCore | null>(null)

  useEffect(() => {
    pGetNewsInformations()
  }, [])
  const handleUpdateLike = useCallback(
    async (
      ...
      e.stopPropagation()
      const index = pNewsInformations.findIndex((el) => el.id === news.id)
      if (index !== -1) {
        dispatch(updateLikeNewsInformations(index))
        const res = await updateLike({
          id: news.id,
          liked: !pNewsInformations[index].hasCurrentUserLiked
        })
      }
    },
    [pNewsInformations]
...
const mapStateToProps = (state: AppState) => ({
  pNewsInformations: state.news.newsInformations
})

const mapDispatchToProps = (dispatch: AppDispatch) => ({
  pGetNewsInformations: () => dispatch(getNewsInformations())
})

export default connect(mapStateToProps, mapDispatchToProps)(WidgetNewsInformations)
