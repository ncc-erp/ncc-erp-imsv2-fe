...
import { getGuidelineInformations } from "@/store/news/thunkApi"
...
import { updateLikeGuidelineInformations } from "@/store/news"
import { WIDGET_PADDING, WIDGET_SPACING, WIDGET_WIDTH } from "../GridLayout"
import Link from '@mui/material/Link';

interface IWidgetGuidelineInformations {
  pGuidelineInformations: INewsRes[]
  pGetGuidelineInformations: () => Promise<unknown>
  row?: number
}
...
const WidgetGuidelineInformations = ({ pGetGuidelineInformations, pGuidelineInformations, row = 0 }: IWidgetGuidelineInformations) => {
  const classes = useStyles()
  const dispatch = useDispatch()
  const swiperRef = useRef<SwiperCore | null>(null)

  useEffect(() => {
    pGetGuidelineInformations()
  }, [])
  ...
  const handleUpdateLike = useCallback(
    async (
      ...
      e.stopPropagation()
      const index = pGuidelineInformations.findIndex((el) => el.id === news.id)
      if (index !== -1) {
        dispatch(updateLikeGuidelineInformations(index))
        const res = await updateLike({
          id: news.id,
          liked: !pGuidelineInformations[index].hasCurrentUserLiked
        })
      }
    },
    [pGuidelineInformations]
  )
  return (
    <SwiperStyle
      enabled={true}
    ...
    >
      {pGuidelineInformations?.map((news) => (
...
const mapStateToProps = (state: AppState) => ({
  pGuidelineInformations: state.news.guidelineInformations
})

const mapDispatchToProps = (dispatch: AppDispatch) => ({
  pGetGuidelineInformations: () => dispatch(getGuidelineInformations())
})

export default connect(mapStateToProps, mapDispatchToProps)(WidgetGuidelineInformations)
